...
const Footer = () => {
  return (
    <div className='w-full h-screen flex bg-zinc-900 p-20'> 
          <div className="w-1/2 h-full font-['Haettenschweiler'] flex flex-col items-centre justify-between">
              <div className="heading">
                    <h1 className="text-[8vw] uppercase -mb-8 leading-none ">Eye-</h1>
              <h1 className="text-[8vw] uppercase -mb-10 leading-none ">Opening</h1>
          ...
          <div className="w-1/2 text-zinc-400">
              <div className=" font-['Haettenschweiler']">
                    <h1 className="text-[8vw] uppercase -mb-8 leading-none text-zinc-100">presentations</h1>
              </div>
              <div className="flex flex-col mt-20">
                  <p className='text-lg mb-5'>S:</p>
                  {["instagram", "twitter", "facebook", "github"].map((items, index) => (
                      <a key={index} className='uppercase font-["Neue_Montreal"] text-md' href={`/${items}.com`}>{ items }</a>
                 ))}
              </div>
              <div className='flex items-center justify-between'>
                  <div className="flex flex-col mt-5">
                  <p className='text-lg mb-5'>L:</p>
                      {["202-1965 W 4th Ave", "Vancouver, Canada"," ", "30 Chukarina St", "Lviv, Ukraine"].map((items, index) => (
                      <a key={index} className='uppercase font-["Neue_Montreal"] text-md' href={`/${items}.com`}>{ items }</a>
                 ))}
                  </div>
                  <div className="flex flex-col mt-5">
                  <p className='text-lg mb-5'>M:</p>
                  {["Home", "service", "about","insight", "contact"].map((items, index) => (
                      <a key={index} className='uppercase font-["Neue_Montreal"] text-md' href={`/${items}.com`}>{ items }</a>
                 ))}
              </div>
              </div>
              <div className="flex flex-col mt-5">
                  <p className='text-lg mb-5'>E:</p>
                  {["hello@ochi.design"].map((items, index) => (
                      <a key={index} className='font-["Neue_Montreal"] text-md' href={`/${items}.com`}>{ items }</a>
                 ))}
              </div>
      </div>
    </div>
